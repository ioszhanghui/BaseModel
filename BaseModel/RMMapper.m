...
+(id)populateObject:(id)obj fromDictionary:(NSDictionary *)dict exclude:(NSArray *)excludeArray {
    ...
    for (NSString *key in [properties allKeys]) {
        if ([[properties objectForKey:key] isEqualToString:@"NSString"]) {
            [obj setValue:@"" forKey:key];
        }
        if ([[properties objectForKey:key] isEqualToString:@"NSArray"]) {
            
            [obj setValue:@[] forKey:key];
        }
        if ([[properties objectForKey:key] isEqualToString:@"NSDictionary"]) {
            [obj setValue:@{} forKey:key];
        }
        if ([[properties objectForKey:key] isEqualToString:@"NSMutableArray"]) {
            NSMutableArray *arr = [[NSMutableArray alloc]initWithCapacity:0];
            [obj setValue:arr forKey:key];
        }
        if ([[properties objectForKey:key] isEqualToString:@"NSMutableDictionary"]) {
            NSMutableDictionary *dic = [[NSMutableDictionary alloc]initWithCapacity:0];
            [obj setValue:dic forKey:key];
        }
    }
        ...
        if ([propertyType isEqualToString:@"NSString"] \
            ...
            // Else, set value for key
                ...
                if ([value isKindOfClass:[NSNumber class]]) {
                    ...
                    [obj setValue:[NSString stringWithRequestDicValue:valueStr defultValue:@""] forKey:property];
                }
                else
                {
                    [obj setValue:[NSString stringWithRequestDicValue:value defultValue:@""] forKey:property];
                }
